...
- 限制为 1 个 IP 时，可根据原 IP 连续 20 秒的实际流量自动完成移动网络与 Wi-Fi 切换。
...
一键创建时会自动选择 `50000-65000` 高位端口，并尝试通过 `ufw`、`firewalld` 或服务器现有的 `host_hardening.nft` 配置自动放行节点端口；Hysteria2 启用端口跳跃时也会放行对应范围。删除节点后，OUI 会自动关闭由本次一键创建实际新增、且未被其他本机节点使用的端口规则。服务器原本已有的手动放行规则不会被 OUI 删除；仅关闭节点时也会保留端口，便于重新启用。
...
关闭订阅服务后，Telegram 一键创建仍会返回可直接使用的独立节点链接和二维码，但不会生成客户端 `subId`，也不会发送订阅页或 JSON 订阅链接。
