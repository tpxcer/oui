...
- **IP 限制自动对接**：安装和更新时会自动尝试启用 Fail2Ban/IP Limit 监控和防火墙链，客户端超过 IP 上限时可断开连接、写入封禁日志，并只封当前节点端口。
...
说明：OUI 负责识别超限 IP、断开客户端并写入端口级防火墙规则；Fail2Ban 仅作为日志监控和辅助状态组件，不再执行全端口封禁。
