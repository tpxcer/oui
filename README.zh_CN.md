...
- **IP 限制自动对接**：安装和更新时会自动尝试启用 Fail2Ban/IP Limit 监控和防火墙链。客户端超过 IP 上限时，只封“超限 IP + 当前节点端口”；限制为 1 时支持根据 20 秒实际流量自动接管，减少手机流量与 Wi-Fi 切换误封。
...
- **中文发布说明**：每个正式版本都应更新 `CHANGELOG.md` 和 `config/release-notes/latest.md`；GitHub Release、面板检测更新、网页一键更新弹窗和 Telegram 更新菜单会同步展示本版本更新内容，Telegram 后台更新开始和更新成功反馈只显示更新状态。
...
- 限制为 `1` 且出现第 2 个 IP 时，会统计原占位 IP 在该入站端口连续 20 秒的双向流量；低于 `100 KiB` 时由新 IP 自动接管，达到或超过阈值时才掐断新 IP。
- 正常网络切换接管不会发送超限掐断通知；三个及以上 IP 同时出现时仍按原规则严格限制。
- 限制为 `1` 时，第 2 个不同 IP 会先进入切换判断；限制为 `3` 时，第 4 个不同 IP 属于超限。
