...
OUI 是面向 Linux VPS 的 Xray 管理面板，保留协议管理、用户管理、流量统计、节点管理、订阅、证书和日志等核心能力，并加入 Telegram 机器人、一键节点创建、IP 限制自动对接、信息预览和网页一键更新能力，让面板更适合快速开节点、观察节点状态和处理异常连接。
...
- **TG 封禁操作反馈**：IP 超限通知中可临时解封、永久解除封禁或手动封禁；永久解除封禁会自动把面板中的 IP 限制数量增加 `1`，成功或失败都会发送 TG 反馈。
- **信息预览页**：将 API 文档入口调整为信息预览，集中展示 VPN 溯源、服务器信息、流量使用情况和资源状态。
- **VPN 溯源查询**：支持通过 IP 查询节点实际使用位置，并拼接展示 IP、地区、坐标和错误等结果。
...
- `解除封禁`：永久解除该 IP 的封禁，并自动把面板里该客户端的 `IP 限制` 增加 `1`。
- `手动封禁`：立即写入端口级封禁规则。
- 解封、封禁成功或失败都会发送 Telegram 反馈。
