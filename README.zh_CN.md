...
- **IP 限制自动对接**：安装和更新时会自动尝试启用 Fail2Ban/IP Limit jail，客户端超过 IP 上限时可断开连接、写入封禁日志，并在 Fail2Ban 正常运行时加入防火墙封禁。
...
客户端的 `IP 限制` 设置为大于 `0` 后，OUI 会按客户端记录最近出现过的 IP。超过上限时，系统会优先保留较早上线的 IP，并对后续超限 IP 执行以下动作：

- 写入 `/var/log/x-ui/3xipl.log`。
- 临时断开该客户端连接，让超限设备掉线。
- 已配置 Telegram 管理员通知时，发送“超出 IP 上限，已掐断”。
- 已启用 Fail2Ban 的 `3x-ipl` jail 时，立即封禁超限 IP。

安装脚本和更新脚本会自动尝试配置 Fail2Ban/IP Limit jail，涉及文件如下：
...
说明：OUI 可以识别超限 IP 并断开客户端；真正的系统防火墙封禁依赖 Fail2Ban 和 `3x-ipl` jail 正常运行。
